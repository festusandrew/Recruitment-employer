...
export function ActiveJobs({ onViewAllJobs, onViewPipeline }: { onViewAllJobs?: () => void; onViewPipeline?: (job: any) => void; }) {
    ...
    return (
        <>
            <div className="bg-white rounded-2xl p-6 border border-slate-100 shadow-premium text-left relative overflow-hidden">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h2 className="text-lg text-slate-900 font-extrabold tracking-tight font-sans">Active Jobs</h2>
                    ...
                    <Button 
                        ...
                        className="hover:bg-indigo-600 hover:text-white hover:border-indigo-600 cursor-pointer font-bold text-xs px-4 py-2 rounded-xl transition-all duration-300 bg-white border-slate-200 text-slate-700"
                    >
                        View All Jobs
                    </Button>
                </div>
                    ...
                    <div className="flex gap-5 pb-4 min-w-max">
                        {jobsList.map((job, idx) => (
                            <motion.div
                                ...
                                whileHover={{ y: -4, boxShadow: "0 12px 20px -8px rgba(99, 102, 241, 0.08)", borderColor: "rgba(99, 102, 241, 0.2)" }}
                                className="bg-white rounded-2xl p-5 border border-slate-100 shadow-premium transition-all duration-300 w-[320px] flex-shrink-0 flex flex-col justify-between h-[280px]"
                            >
                                <div>
                                    {/* Header */}
                                    <div className="flex items-start justify-between mb-3">
                                        ...
                                        >
                                            <h3 className="text-slate-900 font-bold text-sm leading-tight hover:text-indigo-600 transition-colors line-clamp-2">{job.title}</h3>
                                        ...
                                        <button 
                                            className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors cursor-pointer border border-transparent hover:border-slate-200/50"
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                setActionJob(job);
                                ...
                                <div>
                                    ...
                                    <div className="mb-4">
                                        <div className="flex items-center justify-between mb-1.5 text-xs font-semibold text-slate-550">
                                            <div className="flex items-center gap-1.5">
                                                <Users className="w-3.5 h-3.5 text-slate-400" />
                                                <span>{job.applicants} Applicants</span>
                                            </div>
                                            <span className="text-slate-900 font-bold">{job.progress}%</span>
                                        </div>
                                    ...
                                    <div className="flex gap-1 pt-3 border-t border-slate-100">
                                        <Button
                                            variant="ghost"
                                            size="sm"
                                            className="flex-1 text-[10px] font-bold h-8 cursor-pointer hover:bg-indigo-50 hover:text-indigo-600 text-slate-600 rounded-lg transition-all"
                                            onClick={() => {
                                                if (onViewPipeline) {
                                                    onViewPipeline(job);
                                                } else {
            ...
            <EditJobModal
                ...
                onSave={(updatedJob) => {
                    setJobsList(prev => prev.map(j => j.id === updatedJob.id ? { ...j, ...updatedJob } : j));
                    setIsEditModalOpen(false);
                    if (activeView === "list") setSelectedJob(null);
                }}
                job={selectedJob}
